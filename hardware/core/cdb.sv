 ...
 */
module cdb(clk, rst_n, fu_results_i, cdb_o);
    import rv32if_pkg::*;
    ...
    // Select winning FU result
    always_comb begin
        cdb_o = '{valid: 1'b0, tag: '0, value: '0};
        if      (grant_vec[0]) cdb_o = fu_results_i[0];
        else if (grant_vec[1]) cdb_o = fu_results_i[1];
        else if (grant_vec[2]) cdb_o = fu_results_i[2];
        else if (grant_vec[3]) cdb_o = fu_results_i[3];
        else if (grant_vec[4]) cdb_o = fu_results_i[4];
        else if (grant_vec[5]) cdb_o = fu_results_i[5];
        else if (grant_vec[6]) cdb_o = fu_results_i[6];
    end

    always_ff @(posedge clk or negedge rst_n) begin
        if (~rst_n) last_grant <= {{NUM_FU-1{1'b0}}, 1'b1}; // start at slot 0
